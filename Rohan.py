...
async def play(update, context):
    ...
    try:
        ...
        # Retry logic for YouTube object creation
        max_retries = 2
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Retry {attempt + 1}/{max_retries} for URL {url}: {str(e)}")
...
async def lyrics(update, context):
    ...
    try:
        song at genius.search_song(query)
        if song:
...
async def show_playlist(update, context):
   Vote user_id = update.message.from_user.id
