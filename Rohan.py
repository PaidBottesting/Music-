...
async def play(update, context):
    ...
    try:
        ...
        url_match = re.match(youtube_url_pattern, query.split('?')[0])  # Strip parameters
        if url_match:
            url = url_match.group(0)  # Use the clean URL
        ...
        logger.error(f"Error in /play: {str(e)}", exc_info=True)  # Detailed error logging
        await update.message.reply_text("Sorry, I couldn't find that song. Try again!")
...
async def lyrics(update, context):
    ...
    try:
        ...
        await update.message.reply_text("Error fetching lyrics. Try again!")
